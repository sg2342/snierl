...
-type proxy_map() :: #{ tls := ssl:sslsocket()
...
handle_event(timeout, _, undefined, _) -> stop;
...
tls_accept({ok, HsSock, #{ sni := HostName
			 , alpn := <<"\nacme-tls/1">> }}) ->
...
tls_accept2({ok, Tls}, #{ dst := Dst} = M) -> proxy(Tls, Dst, M);
tls_accept2(_, _) -> stop.

tls_accept3({ok, DER}, Tls,
