...
handle_event(state_timeout, _, _State, M) ->
    check_for_action(M);
handle_event(
    {call, From},
    {alpn_lookup, Name},
            ...
            Cert ->
                {Name, #{
                    hs_opts => [
                        {key, Key},
                        {cert, Cert},
                        {alpn_preferred_protocols, [<<"acme-tls/1">>]}
                    ]
                }}
        end,
...
handle_event(
    ...
    _State,
    #{alpn_key := Key, alpn_validation := V} = M
) ->
    Name = binary_to_list(BinName),
    Cert = snierl_crts:alpn(Key, Name, KeyAuth),
    {keep_state, M#{alpn_validation => V#{Name => Cert}}, [{reply, From, {ok, Name}}]};
...
do_insert1(
    {Name, #{hs_opts := acme, hs_extra := Extra}},
    #{hs_opts := Opts} = Map
) ->
...
extra_hs_opts([{cacertfile, _} = V | T], Opts) -> extra_hs_opts(T, extra_hs_opts1(V, Opts));
extra_hs_opts([{cacerts, _} = V | T], Opts) -> extra_hs_opts(T, extra_hs_opts1(V, Opts));
...
check_for_action2(false, H, _, _, Hosts) ->
    ets:delete(?TAB, H),
    Hosts;
...
check_for_action3([{H, #{expires := Ex}}], Cutoff, Account) when
    Ex < Cutoff
->
    acme_worker(Account, H);
check_for_action3(_, _, _) ->
    ok.
